...
//    }
    ...
    Paper.prototype.freeTransform = function(subject, options, callback) {
        ...
        if (options.subjectBBox !== undefined) {
            bbox = options.subjectBBox;
        }
...
//test!!
            ...
            ft.group.attr({ id: subject.attr("id") + "_ft" });
            ft.axes.map(function(axis) {
                ft.handles[axis] = {};
        ...
        ft.setOpts = function(options, callback) {
            ...
            var i, j;
            if (options.subjectBBox !== null && options.subjectBBox !== undefined) {
                ft.opts.subjectBBox = options.subjectBBox;
            }
        ...
        ft.hideResizeHandler = function () {
            if (ft.group &&
                (!ft.group.hasClass("hide"))) {
                ft.group.addClass("hide");
            }
        }

        ft.showResizeHandler = function () {
            if (ft.group &&
                ft.group.hasClass("hide")) {
        ...
        ft.isShowResizeHandler = function () {
            if (ft.group && ft.group.hasClass("hide")) {
                return false;
            }
            return true;
        }
        ...
        function applyLimits(bbox) {
            ...
            if ( ft.opts.boundary ) {
				ft.opts.boundarywidth  = ft.opts.boundary.width;
                ft.opts.boundaryheight = ft.opts.boundary.height;
				...
				if ( ft.attrs.center.y + ft.attrs.translate.y < ft.opts.boundary.y ) {
					ft.attrs.translate.y += ft.opts.boundary.y - ( ft.attrs.center.y + ft.attrs.translate.y );
				}
				...
				if ( ft.attrs.center.y + ft.attrs.translate.y > ft.opts.boundary.y + ft.opts.boundaryheight ) {
					ft.attrs.translate.y += ft.opts.boundary.y + ft.opts.boundaryheight - ( ft.attrs.center.y + ft.attrs.translate.y );
                }
			}
