...
var bDebug 		= true;    // TPLSvc 디버깅 모드 (true 설정 시 페이지에 textBox 생성)
...
/**
 * TPL port info
 * 연결 가능한 port를 찾을때 시작 port  (http는 7895~7935, https는 7936~7966, ws는 7970~7981, wss는 7982~7992)
 */
var minport     = 7895;       // 연결 가능한 port를 찾을때 최소 범위
var maxport     = 7900;       // 연결 가능한 port를 찾을때 최대 범위    
    
//WebSocket 사용
var objWebSocket;                                      //WebSocket Object
var bWebsocketUse = true;                              //WebSocket 사용 유무: true:사용, false:사용안함.
var szprotocol = window.location.protocol;             //WebSocket protocol 스트링.

// WebSocket 사용 유무에 따라 protocol, minport, maxport 설정 
if(bWebsocketUse)
{
	if(szprotocol == "http:")
	{
...
function fnTplStart() {
	...
	var arrProd = ["miplatform", "xplatform", "nexacro"];
	    strProductCode = strProductName.toLowerCase();
	if(arrProd.indexOf(strProductCode) < 0) {
		alert("Product Name이 잘못되었습니다. strProductName 을 확인하세요");
		return;
	}
	trace("fntplStart:" + strProductCode);
	openurl_add = "/launcher/" + strProductCode;  //고정(변경불가) "/launcher/xplatform", "/launcher/nexacro" openurl_add = "/launcher/" + strProductCode;  //고정(변경불가) "/launcher/xplatform", "/launcher/nexacro" /launcher/miplatform
   // objProduct.platform = strProductCode; //필수 세팅  
	
		objProduct.platform = "nexacro"; //필수 세팅  (launcher , engine check를 공통사용후 실제 설치시 변경함) 
		
	var vObj = TOBE_PRODUCTS[strProductCode];
	if(strProductVersion == "17.0") strProductVersion = "17"; 
	if(strProductVersion == "N") strProductVersion = "21";
        ...
        if (action == "create") {
            ...
            if (result.result == "success") {         	
            	if(bMiPlatformInstallCheck) {
            		isJsonp = true;
                    do_property('setproperty', 'basicmp'); // mp 구동을 위한 기본 속성 설정
					//do_property('setproperty', 'basic'); // mp 구동을 위한 기본 속성 설정
                    sendData(true);   
                    return;
            	}
            ...
            if (result.result == "success") {
                if (result.value.makeshortcut) {
                    if (typeof(bAddWebInfo) !== undefined && bAddWebInfo === true) {
				...
				}else if (result.value.MakeShortCut) {	//mp 만해당
						if(bMiPlatformInstallCheck) {
							do_method('download'); // 다운로드 생성 
							sendData(true);
							return;
						}
						...
						if(bMiPlatformInstallCheck) {
							do_method('launch'); // 다운로드 완료 되면 실행 
							sendData(true);
							return;
						}
				  } 
                    ...
                    if (!bUpdateCheck) {
						...
						if(strProductName == "miplatform" && bMiPlatformInstallCheck == false) 
						{
                            bEngineCheck = true;
                            tpl_finish();
                    		istalledMsg("["+strProductName+"]");
                    		setTimeout(function() {
	                        	objProduct  = {};
	                        	bMiPlatformInstallCheck = true;
	                            tpl_start(true); //create / install
	                                        	
	                        }, 500);
	                        return;
                    	}
						if(strProductName == "xplatform" && bEngineCheck == false) {
                            bEngineCheck = true;
                            tpl_finish();
                    		istalledMsg("["+strProductName+"]");
                    		setTimeout(function() {
				  ...
				  else if (result.value.launch == true || 
						   result.value.launch == "ok" || 
						   result.value.Run == "ok") {
                    setTimeout(function() {
                        tpl_finish();
                    }, 1000);
                } else if (result.value.StartDownload) {
							do_method('launch'); // 다운로드 완료 되면 mp실행 
							sendData(true);
							return;
				}
            } else {
...
function tpl_start() {
    //생성 및 id 할당
    trace("tpl_start", "생성 및 id 할당"+ strProductCode);
    ...
    if(vPrd == "miplatform" && bMiPlatformInstallCheck == false) {
        ...
        openurl_add = "/launcher/" + strProductCode;  //고정(변경불가) "/launcher/xplatform", "/launcher/nexacro" /launcher/miplatform
        objProduct.platform = strProductCode; //필수 세팅
    } else {
    	objProduct.platform = strProductCode; //필수 세팅
    }
...
function sendData(openpost, resultCallback) {
	...
	if(bWebsocketUse){
		var jsonData;
			...
			//trace(objProduct.action+ ">>>>>>>>> senddata:"+ jsonData);
		if( objWebSocket != null) {
			//trace("tpl Websocket Return senddata:"+ jsonData);
...
function do_property(action, action_sub) {
    if (action == 'setproperty') { //property 설정 ==> action = 'property' value = {"property명" : "property값", ... };
        ...
        var vPrd = strProductName.toLowerCase();
	if(vPrd == "miplatform" || vPrd == "xplatform"  ) {
...
/**
 * [fn_checkVersion]
 * 런처 버전 비교 로직 추가 - 2020.02.06 추가(versionB(local)이 낮으면 false)  --> 다운로드 함
 */
...
function onMessage(evt)
{
	...
	if(launcher_response(data)) {
		if(debugmode.loglevel=="debug") {
			trace("tpl Websocket Return onMessage  -\n",data);
			trace("======================================================================");
		}
		var result = JSON.parse(data);
		httpCallBackReturn(result);
...
{
	if(debugmode.loglevel=="debug"||debugmode.loglevel=="error") {
	  trace("tpl Websocket onerror ", "first line", "action : " + objProduct.action, "oepnport : "+openport);
	}
	...
	if (objProduct.action == 'create') {
		...
		if (openport <= maxport) {
		  //sendData(true);
		  if(debugmode.loglevel=="debug"||debugmode.loglevel=="error") {
			trace("tpl Websocket onerror ", ">>>>>>>>>>>>>>>>>> Next Port Connect", "action : " + objProduct.action, "oepnport : "+openport);
		  }
		  objWebSocket = null;
		  tpl_WebSocketConnect();
		} else {
